  ...
  ({ combination, dishes = null, combinationId = null, editFn }) => {
    ...
    const extrasTotalPrice = useMemo(() => {
      return Object.keys(selectedDishes).reduce((acc, key) => {
        return (
          acc +
          selectedDishes[key].reduce((sum, dishItem) => {
            return (
              sum +
              dishItem.extrass.reduce(
                (extrasSum, ext) => extrasSum + ext.price,
                0
              )
            );
          }, 0)
    ...
    const createInitState = useCallback(() => {
      return combination.combinationDishes.reduce(
        (acc, el) => ({ ...acc, [el.name]: [] }),
        {}
      );
    ...
    const handleUpdateSelectedDishes = (
      key: string,
      qty: number,
      dish: IDish
    ) => {
      ...
      const dishIndex = selectedDishes[key]?.findIndex(
        (el) => el.dish.id === dish.id
      );
    ...
    const handleUpdateExtrass = (
      ...
      setSelectedDishes((curr) => {
        ...
        const extraIndex = findedDish.extrass.findIndex(
          (el) => el.id === extra.id
        );
    ...
    return (
      <div className={styles.combination}>
        ...
        <div className={styles.footer}>
          ...
          >
            {dishes ? texts.edit || 'Edit' : texts.add || 'Add to cart'}
          </button>
        </div>
      </div>
    );
