...
import {
  selectCombinations,
  ICombinationItem as ICombinationFromSlice,
...
export const CombinationComponent: React.FC<IProps> = memo(
  ({ isOpen = false, combination, toggleOpen }) => {
    ...
    const [combinationToUpdate, setCombinationToUpdate] =
      useState<ICombinationFromSlice | null>(null);
    ...
    useEffect(() => {
      ...
      if (itemContainerRef.current) {
        itemContainerRef.current.scrollIntoView({
          behavior: 'auto',
          block: 'start',
        });
      }
    ...
    return (
      <div className={styles.category} ref={itemContainerRef}>
        ...
        >
          ...
          <span
            className={`${styles.icon} ${isOpen ? styles.iconReverted : ''}`}
          >
            <ExpandMoreIcon />
          </span>
        </button>
        <div
          ref={itemBoxRef}
          className={`${styles.list} ${isOpen ? styles.listExpanded : ''}`}
        >
          <div className={styles.combinationInner}>
            ...
            <div className={styles.combinationList} id='combinationList'>
              {Boolean(relatedCombinations.length) && (
                <>
                  {relatedCombinations
                    .filter((el) => el.id !== combinationToUpdate?.id)
                    .map((el) => (
                      <CartItemComponent
          ...
          <div className={styles.combinationFooter}>
            <button
              ...
              disabled={
                !Boolean(relatedCombinations.length) ||
                count > relatedCombinations.length
              }
            >
              {texts['add.more.combination'] || 'Add more menu del dia'}
            </button>
          </div>
        </div>
      </div>
